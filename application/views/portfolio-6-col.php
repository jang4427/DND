<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">

    <title>Portfolio</title>

    <!-- Bootstrap Core CSS -->
    <link href="http://127.0.0.1:8888/assets/css/bootstrap.min.css" rel="stylesheet">

    <!-- Custom CSS -->
    <link href="http://127.0.0.1:8888/assets/css/main.css" rel="stylesheet">

    <!-- Responsive CSS -->
    <link href="http://127.0.0.1:8888/assets/css/responsive.css" rel="stylesheet">


    <!-- Custom Fonts -->
    <link href="http://127.0.0.1:8888/assets/font-awesome-4.1.0/css/font-awesome.min.css" rel="stylesheet" type="text/css">
    <link href='http://fonts.googleapis.com/css?family=Oswald:400,300,700' rel='stylesheet' type='text/css'>
    <link href='http://fonts.googleapis.com/css?family=Lato:300,400,700' rel='stylesheet' type='text/css'>
    <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
    <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
    <script src="https://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->

</head>

<body>

<!-- Navigation -->
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
    <div class="container">
        <!-- Brand and toggle get grouped for better mobile display -->
        <div class="navbar-header">
            <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1">
                <span class="sr-only">Toggle navigation</span>
                <span class="icon-bar"></span>
                <span class="icon-bar"></span>
                <span class="icon-bar"></span>
            </button>
            <a class="navbar-brand" href="http://127.0.0.1:8888/index.html"><img src="http://127.0.0.1:8888/assets/img/logo.png" alt=""/></a>
        </div>
        <!-- Collect the nav links, forms, and other content for toggling -->
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav navbar-right">
                <li>
                    <a href="http://127.0.0.1:8888/controlls">Home</a>
                </li>
                <li>
                    <a href="http://127.0.0.1:8888/controlls/about_us">Work list</a>
                </li>
                <li>
                    <a href="http://127.0.0.1:8888/controlls/portfolio">Portfolio</a>
                </li>
<!--                <li>-->
<!--                    <a href="http://127.0.0.1:8888/controlls/news">News</a>-->
<!--                </li>-->
                <li>
                    <a href="http://127.0.0.1:8888/controlls/contact">Contact</a>
                </li>
            </ul>
        </div>
        <!-- /navbar-collapse -->
    </div>
    <!-- /container -->
</nav>
<!-- /navigation -->


<!-- Portfolio Banner -->
<header class="portfolio1-banner">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <h2 class="main-text"><span class="part"> Portfolio</span></h2>
                <div class="intro-text">view our work</div>
            </div>
        </div>
    </div>
</header>


<div class="container">

    <div class="row margin-top">
        <div class="col-md-7">
            <a href="portfolio-item.html">
                <img class="img-hover" src="http://127.0.0.1:8888/assets/img/26.jpg" alt="photo"/>
            </a>
        </div>
        <div class="col-md-5">
            <div class="portfolio-header">Project One
            </div>
            <div class="portfolio-subheader">Subheading
            </div>
            <p>You cannot simply put something new into a place. You have to absorb what you see around you, what exists on the land, and then use that knowledge along with contemporary thinking to interpret what you see.</p>
            <a class="btn portfolio-btn" href="portfolio-item1.html">View Project</a>
        </div>
    </div>

    <hr>

    <div class="row">
        <div class="col-md-7">
            <a href="portfolio-item.html">
                <img class="img-hover" src="http://127.0.0.1:8888/assets/img/27.jpg" alt="photo"/>
            </a>
        </div>
        <div class="col-md-5">
            <div class="portfolio-header">Project Two
            </div>
            <div class="portfolio-subheader">Subheading
            </div>
            <p>Space, space: architects always talk about space! But creating a space is not automatically doing architecture. With the same space, you can make a masterpiece or cause a disaster.</p>
            <a class="btn portfolio-btn" href="portfolio-item1.html">View Project</a>
        </div>
    </div>

    <hr>

    <div class="row">
        <div class="col-md-7">
            <a href="portfolio-item.html">
                <img class="img-hover" src="http://127.0.0.1:8888/assets/img/28.jpg" alt="photo"/>
            </a>
        </div>
        <div class="col-md-5">
            <div class="portfolio-header">Project Three
            </div>
            <div class="portfolio-subheader">Subheading
            </div>
            <p>You cannot simply put something new into a place. You have to absorb what you see around you, what exists on the land, and then use that knowledge along with contemporary thinking to interpret what you see.</p>
            <a class="btn portfolio-btn" href="portfolio-item1.html">View Project</a>
        </div>
    </div>

    <hr>

    <div class="row">
        <div class="col-md-7">
            <a href="portfolio-item.html">
                <img class="img-hover" src="http://127.0.0.1:8888/assets/img/29.jpg" alt="photo"/>
            </a>
        </div>
        <div class="col-md-5">
            <div class="portfolio-header">Project Four
            </div>
            <div class="portfolio-subheader">Subheading
            </div>
            <p>Space, space: architects always talk about space! But creating a space is not automatically doing architecture. With the same space, you can make a masterpiece or cause a disaster.</p>
            <a class="btn portfolio-btn" href="portfolio-item1.html">View Project</a>
        </div>
    </div>

    <hr>

    <div class="row">
        <div class="col-md-7">
            <a href="portfolio-item.html">
                <img class="img-hover" src="http://127.0.0.1:8888/assets/img/30.jpg" alt="photo"/>
            </a>
        </div>
        <div class="col-md-5">
            <div class="portfolio-header">Project Five
            </div>
            <div class="portfolio-subheader">Subheading
            </div>
            <p>You cannot simply put something new into a place. You have to absorb what you see around you, what exists on the land, and then use that knowledge along with contemporary thinking to interpret what you see.</p>
            <a class="btn portfolio-btn" href="portfolio-item1.html">View Project</a>
        </div>
    </div>



    <div class="row text-center">
        <div class="col-md-12">
            <ul class="pagination">
                <li class="active">
                    <a href="http://127.0.0.1:8888/controlls/portfolio/1">1</a>
                </li>
                <li>
                    <a href="http://127.0.0.1:8888/controlls/portfolio/2">2</a>
                </li>
                <li>
                    <a href="http://127.0.0.1:8888/controlls/portfolio/3">3</a>
                </li>
                <li>
                    <a href="http://127.0.0.1:8888/controlls/portfolio/4">4</a>
                </li>
                <li>
                    <a href="http://127.0.0.1:8888/controlls/portfolio/5">5</a>
                </li>
                <li>
                    <a href="http://127.0.0.1:8888/controlls/portfolio/6">6</a>
                </li>
                <li>
                    <a href="http://127.0.0.1:8888/controlls/portfolio/7">7</a>
                </li>
            </ul>
        </div>
    </div>
    <div class="margin-bottom"></div>
</div>


<!-- Footer -->
<div id="footer">
    <div class="footerTop">
        <div class="container">
            <div class="row">
                <!-- Footer Box1 -->
                <div class="col-md-3 col-sm-3"><div class="logoBottom"><i class="fa fa-cubes"></i> architect</div></div>
                <!-- Footer Box2 -->
                <div class="col-md-3 col-sm-3">
                    <h5>Contact info</h5>
                    <p>22 Central Park South, New York, NY 10019</p>
                    <p>Call us: 212-222-4000</p>
                    <p>Email: <a href="mailto:architecture@office.com">architecture@office.com</a></p>
                </div>

                <!-- Footer Box3 -->
                <div class="col-md-3 col-sm-3">
                    <h5>Follow us</h5>
                    <p>Space, space: architects always talk about.</p>
                    <div class="footerLinkList">
                        <a href="#" class="icons-res"><i class="fa fa-facebook fa-2x"></i></a>
                        <a href="#" class="icons"><i class="fa fa-twitter fa-2x icon1"></i></a>
                        <a href="#" class="icons"><i class="fa fa-instagram fa-2x icon1"></i></a>
                        <a href="#" class="icons"><i class="fa fa-linkedin fa-2x icon1"></i></a>
                    </div><!-- /footerLinkList -->
                </div>
                <!-- Footer Box4 -->
                <div class="col-md-3 col-sm-3">
                    <h5>More links</h5>
                    <p>Space, space: architects always talk about.</p>
                    <div class="footerLinkList">
                        <ul>
                            <li><a href="http://127.0.0.1:8888/faq.html">Faq</a></li>
                            <li><a href="team.html">Team</a></li>
                        </ul>
                    </div><!-- /footerLinkList -->
                </div>

            </div><!-- /row -->
        </div><!-- /container -->
    </div><!-- /footerTop -->
    <div class="footerBottom">
        <div class="container">
            <div class="row">
                <div class="col-md-6 col-sm-6 col-xs-6">
                    <div class="footerMenu">
                        <ul>
                            <li><a href="http://127.0.0.1:8888/controlls/about_us">Work list</a></li>
<!--                            <li><a href="http://127.0.0.1:8888/controlls/news">News</a></li>-->
<!--                            <li><a href="services1.html">Services</a></li>-->
                            <li class="last"><a href="http://127.0.0.1:8888/controlls/portfolio">Portfolio</a></li>
                        </ul>
                    </div><!-- /footerMenu -->
                </div>
                <div class="col-md-6 col-sm-6 col-xs-6">
                    <div class="copy">Copyright © 2016. Designed by <span><a href="http://www.marijazaric.com/" target="_blank">creative simplicity</a></span>
                    </div>
                </div>
            </div><!-- /row -->
        </div><!-- /container -->
    </div><!-- /footerBottom -->
</div><!-- /footer -->


<!-- jQuery Version 1.11.0 -->
<script src="http://127.0.0.1:8888/assets/js/jquery-1.11.0.js"></script>

<!-- Bootstrap Core JavaScript -->
<script src="http://127.0.0.1:8888/assets/js/bootstrap.min.js"></script>

<!-- Smooth scroll JS -->
<script src="http://127.0.0.1:8888/assets/js/smoothscroll.js"></script>


</body>

</html>
